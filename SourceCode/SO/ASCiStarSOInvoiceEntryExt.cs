...
namespace ASCISTARCustom.SO
{
    public class ASCiStarSOInvoiceEntryExt : PXGraphExtension<SOInvoiceEntry>
    {
        ...
        public void ReleaseInvoiceProc(List<PX.Objects.AR.ARRegister> list, bool isMassProcess, Action<List<PX.Objects.AR.ARRegister>, bool> baseMethod)
        {
            PXGraph.InstanceCreated.AddHandler<JournalEntry>(graph => graph.RowInserted.AddHandler<Batch>(args =>
            {
                ...
                foreach (var arRegister in list)
                {
                    ...
                    {
                        var customerAllowances = SelectFrom<ASCIStarCustomerAllowance>
                            .Where<ASCIStarCustomerAllowance.customerID.IsEqual<@P.AsInt>
                            .And<ASCIStarCustomerAllowance.orderType.IsEqual<@P.AsString>>>
                    ...
                    if (matchingAllowanceDetails.Any())
                    {
                        ...
                        {
                            var allowanceItem = SelectFrom<ASCIStarCustomerAllowance>
                                .Where<ASCIStarCustomerAllowance.customerID.IsEqual<@P.AsInt>
                                .And<ASCIStarCustomerAllowance.inventoryID.IsEqual<@P.AsInt>>>.View.Select(graph, arRegister.CustomerID, item.InventoryID).TopFirst;
