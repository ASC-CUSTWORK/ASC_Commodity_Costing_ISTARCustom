...
namespace ASCJSMCustom.INKit.Descriptor
{
    ...
    /// </summary>
    public class ASCJSMCostAssignmentAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXRowInsertedSubscriber, IPXRowDeletedSubscriber
    {
        #region Events
        public void RowInserted(PXCache sender, PXRowInsertedEventArgs e)
        {
            ...
            {
                ProcessRowInserted<INKitSpecNonStkDet, ASCJSMINKitSpecNonStkDetExt>(hdrCache, nonStkDet);
            }
            else if (e.Row is INKitSpecStkDet stkDet)
            {
                ProcessRowInserted<INKitSpecStkDet, ASCJSMINKitSpecStkDetExt>(hdrCache, stkDet);
            }
        }
        ...
        public void RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            if (e.Row is INKitSpecNonStkDet nonStkDet)
            {
                ...
                ProcessRowUpdated<INKitSpecNonStkDet, ASCJSMINKitSpecNonStkDetExt>(hdrCache, nonStkDet, oldRow);
            }
            else if (e.Row is INKitSpecStkDet stkDet)
            {
                var oldRow = e.OldRow as INKitSpecStkDet;
                ProcessRowUpdated<INKitSpecStkDet, ASCJSMINKitSpecStkDetExt>(hdrCache, stkDet, oldRow);
            }
        }
        ...
        public void RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
        {
            ...
            if (e.Row is INKitSpecNonStkDet nonStkDet)
            {
                ProcessRowDeleted<INKitSpecNonStkDet, ASCJSMINKitSpecNonStkDetExt>(hdrCache, nonStkDet);
            }
            else if (e.Row is INKitSpecStkDet stkDet)
            {
                ProcessRowDeleted<INKitSpecStkDet, ASCJSMINKitSpecStkDetExt>(hdrCache, stkDet);
            }
        }
        #endregion
        ...
        public static void ProcessRowInserted<TDac, TDacExt>(PXCache hdrCache, TDac row)
            ...
            where TDacExt : PXCacheExtension<TDac>, IASCJSMCostRollup
        {
        ...
        public static void CostAddition(PXCache cache, INKitSpecHdr currentRow, string rollupType, decimal? value)
        {
            ...
            var rowExt = PXCache<INKitSpecHdr>.GetExtension<ASCJSMINKitSpecHdrExt>(currentRow);
            switch (rollupType)
            {
                case CostRollupType.PreciousMetal:
                    {
                        var result = rowExt.UsrPreciousMetalCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPreciousMetalCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Fabrication:
                    {
                        var result = rowExt.UsrFabricationCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrFabricationCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Packaging:
                    {
                        var result = rowExt.UsrPackagingCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPackagingCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.PackagingForLabor:
                    {
                        var result = rowExt.UsrPackagingLaborCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPackagingLaborCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Labor:
                    {
                        var result = rowExt.UsrLaborCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrLaborCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Materials:
                    {
                        var result = rowExt.UsrOtherMaterialsCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrOtherMaterialsCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Freight:
                    {
                        var result = rowExt.UsrFreightCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrFreightCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Handling:
                    {
                        var result = rowExt.UsrHandlingCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrHandlingCost>(currentRow, result);
                    }
                    break;
                case CostRollupType.Duty:
                    {
                        var result = rowExt.UsrDutyCost + value;
                        cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrDutyCost>(currentRow, result);
                    }
                    break;
                default:
                    break;
        ...
        public static void CostValueZeroing(PXCache cache, INKitSpecHdr currentRow, string rollupType)
        {
            ...
            {
                case CostRollupType.PreciousMetal:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPreciousMetalCost>(currentRow, 0m);
                    break;
                case CostRollupType.Fabrication:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrFabricationCost>(currentRow, 0m);

                    break;
                case CostRollupType.Packaging:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPackagingCost>(currentRow, 0m);

                    break;
                case CostRollupType.PackagingForLabor:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrPackagingLaborCost>(currentRow, 0m);
                    break;
                case CostRollupType.Labor:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrLaborCost>(currentRow, 0m);

                    break;
                case CostRollupType.Materials:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrOtherMaterialsCost>(currentRow, 0m);

                    break;
                case CostRollupType.Freight:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrFreightCost>(currentRow, 0m);

                    break;
                case CostRollupType.Handling:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrHandlingCost>(currentRow, 0m);

                    break;
                case CostRollupType.Duty:
                    cache.SetValueExt<ASCJSMINKitSpecHdrExt.usrDutyCost>(currentRow, 0m);
                    break;
                default:
                    break;
            }
