...
using static ASCJSMCustom.Cost.Descriptor.ASCJSNSymbols;
...
namespace ASCJSMCustom.Common.Services.REST
{
    ...
    /// </summary>
    public class ASCJSMMetalsAPILatestRateService : IASCJSMMetalsAPILatestRateService
    {
        private readonly IASCJSMRESTService _starRESTService;
        private readonly PXGraph _graph;

        public ASCJSMMetalsAPILatestRateService(PXGraph graph)
        {
            _graph = graph;
            _starRESTService = new ASCJSMRESTService(_graph);
        }
        ...
        public virtual decimal GetInverseRateValue(string currency, string symbol)
        {
            ...
            {
                throw new PXException(ASCJSMMessages.Error.SymbolNotSpecified);
            }

            var result = _starRESTService.Get<ASCJSMLatestRatesModel>(ASCJSMEndpoints.LatestRates, Parameters(currency, symbol));
        ...
        private static KeyValuePair<string, string>[] Parameters(string currency, string symbol) =>
            ...
            {
                new KeyValuePair<string, string>(ASCJSMQueryParams.Base, currency),
                new KeyValuePair<string, string>(ASCJSMQueryParams.Symbols, symbol),
            };
